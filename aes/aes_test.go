...
func Test_expandKey(t *testing.T) {
	for i, tt := range expandKeyTestSet {
		...
		for j, v := range enc {
			for k, u := range v {
				if u != tt.enc[j][k] {
					t.Errorf("key %d: enc[%d][%d] = %#x, expected %#x", i, j, k, u, tt.enc[j][k])
				}
			}
		}
		for j, v := range dec {
			for k, u := range v {
				if u != tt.dec[j][k] {
					t.Errorf("key %d: dec[%d][%d] = %#x, expected %#x", i, j, k, u, tt.dec[j][k])
				}
			}
		}
	}
...
func Test_Encrypt(t *testing.T) {
	for i, tt := range aesCipherTests {
		...
		if !bytes.Equal(got, tt.enc) {
			t.Errorf("#%d: aesCipher.Encrypt() = %032x, expected %032x", i, got, tt.enc)
		}
	}
}
...
func Test_Decrypt(t *testing.T) {
	for i, tt := range aesCipherTests {
		...
		if !bytes.Equal(got, tt.dec) {
			t.Errorf("#%d: aesCipher.Decrypt() = %032x, expected %032x", i, got, tt.dec)
		}
	}
}
